...
$('.add_column').on('click', addColumns);
$('.remove_column').on('click', removeColumns)

function addColumns() {
  var $testTable = $('.test-table');
  console.log($testTable.find('col.name:last'));
  //add col tag
  $testTable.find('col.name:last').before('<col class="data" />');

  // Add cols
  $testTable.find('tr').each(function() {
    var $this = $(this);
    var colLength = $this.find('td').length;

    $(this).find('td:last-child').before('<td>' + colLength + ' <span>(added)</span></td>');
  });
}
...
function removeColumns() {
  var $testTable = $('.test-table');
  //remove col tag
  $testTable.find('col.name:last').prev().remove();
